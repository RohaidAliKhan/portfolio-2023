...
export default function hoverCards() {
  const container = document.querySelector(".serviceSec .row");
  const containerW = container.clientWidth;

  const cards = document.querySelectorAll(".service-card");
  const cardsLength = cards.length;

  const cardContent = document.querySelectorAll(".service-card .content");

  let currentPortion = 0; // No portion hovered at the start

  cards.forEach((card) => {
    gsap.set(card, {
      xPercent: (Math.random() - 0.5) * 5,
      yPercent: (Math.random() - 0.5) * 5,
